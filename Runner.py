...
class Runner:
    ...
    def build_graph_and_save_to_file(self, bases_tuple):
        ...
        if not graph.has_vertex(dest_vertex):
            color_print('{} and {} are not able to exchange '.format(base_a, base_b), Colors.RED)
        else:
            color_print('{} and {} are able to exchange '.format(base_a, base_b), Colors.GREEN)
