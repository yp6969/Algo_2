...
class BinMatrix:
    def __init__(self, m=np.array([[1]])):
        """
		Initilize a matrix.
		"""
        self.m = m
    ...
    def is_base2(self):
        ...
        """
        for i in range(3, self.r_len + 1):
            comb = combinations(self.m, i)
            if not self.__is_non_zero_vector_combination(comb):
                return False
        return True

    def __is_non_zero_vector_combination(self, vector_combination) -> bool:
        """
        get all the combination of verctors in size n
        and return FALSE if there is any zero vector result
        """
        for vector_set in vector_combination:
            if not self.__is_non_zero_result(vector_set):
                return False
        return True

    def __is_non_zero_result(self, vector_set) -> bool:
        """
        takes set of vectors and check if the addition of them
        doesn't return zero vector
        """
        for col in zip(*vector_set):
            if self.__binary_add(*col) == 1:
                return True
        return False

    @staticmethod
    def __binary_add(*numbers):
        """
        add binary numbers
        return: 0,1
        """
